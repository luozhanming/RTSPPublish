...
public class StreamServer
    ...
    protected final String TAG = "oywf";
    ...
    private static final double MJPEG_MAX_FPS = 10;
    ...
    public StreamServer(@NotNull Context context, int port) throws IOException {
        ...
        if (ServiceConfig.isServerUpdateDDNS) {
            String hostname = ServiceConfig.ddns_host;
            String username = ServiceConfig.ddns_username;
            String password = ServiceConfig.ddns_password;
            switch ("noip") {
                case "noip":
    ...
    private void setupDDNSClient() {
        if (mDDNSClient != null) {
            String network = ServiceConfig.serverDDNSNetworkType;
    ...
    public void push(VideoFrame frame) throws InterruptedException {
        Log.d(TAG, "push frame: "+frame.getTimestamp());
        if (frame.isConfig()) {
            // Save SPS and PPS
    ...
    private void sendMJPEGStream(final StreamConnection connection, String fps)
        ...
        double mjpegFps = Utils.coerce(
                Utils.tryParseDouble(fps, MJPEG_MAX_FPS),
                MJPEG_MIN_FPS, MJPEG_MAX_FPS);
